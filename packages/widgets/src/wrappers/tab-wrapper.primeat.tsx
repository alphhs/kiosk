...
export function TabWrapper({ items, tabOptions, textConfig, extraBlocks }: TabWrapperProps) {
  ...
  return (
    <Tabs defaultValue={firstItem}>
      <section className={cn('flex min-w-0', getDirection(isVertical, isReverse), 'gap-4')}>
        <div
          className={cn(
            'border-base-300/20',
            {
            ...
            >
              {items.map((tab) => (
                <TabsTrigger
                  ...
                  value={tab.id}
                  className={cn(
                    'hover:border-base-300/60 data-[state=active]:border-accent data-[state=active]:text-base-content border-transparent',
                    'shrink-0',
                    { ['data-[state=active]:bg-primary']: tabOptions?.isCard },
        ...
        {items.map((c) => {
          const content = { ...c, extraBlocks };
          return (
            <TabsContent key={c.id} value={c.id} className="grow">
